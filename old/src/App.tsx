...
/** Type 1: Look at picture, fill blanks (original exercise 1) */
...
/** Type 2: Translate sentences into English */
...
/** Type 3: Write full sentences (what are the kids doing) */
interface ExerciseTypeWrite {
  ...
  questions: {
    id: number;
    prompt: string; // e.g. "Anna ... (wear) a mac."
    correctAnswers: string[];
  }[];
}

/** Type 4: Choose the right verb: am / is / are */
interface ExerciseTypeChooseVerb {
  ...
  questions: {
    id: number;
    sentence: string; // e.g. "The baby (1) sleeping."
    correctAnswers: string[];
  }[];
}

/** Type 5: Fill blanks with -ing form (same as picture but without picture) */
...
const exercises: Exercise[] = [
  ...
  // ── Exercise 2: Translate sentences ───────────────────────────────────────
  {
    ...
    questions: [
      {
        id: 1,
        russianSentence: "Он пьет колу.",
        correctAnswers: ["He is drinking a Coke.", "He is drinking Coke.", "He is drinking cola.", "He is drinking a cola."],
      },
      {
        id: 2,
        russianSentence: "Мы играем в игру.",
        correctAnswers: ["We are playing a game.", "We are playing the game."],
      },
      {
        id: 3,
        russianSentence: "Я катаюсь на машине.",
        correctAnswers: ["I am riding a car.", "I am driving a car.", "I'm riding a car.", "I'm driving a car."],
      },
      {
        id: 4,
        russianSentence: "Она читает книгу.",
        correctAnswers: ["She is reading a book.", "She is reading the book."],
      },
      {
        id: 5,
        russianSentence: "Они бегут по парку.",
        correctAnswers: ["They are running in the park.", "They are running through the park.", "They are running around the park."],
      },
    ],
  },
  ...
  // ── Exercise 3: Write full sentences ──────────────────────────────────────
  {
    ...
    questions: [
      {
        id: 1,
        prompt: "Anna ... (wear) a mac.",
        correctAnswers: ["Anna is wearing a mac.", "Anna is wearing a Mac."],
      },
      {
        id: 2,
        prompt: "Ben ... (ride) a bike.",
        correctAnswers: ["Ben is riding a bike."],
      },
      {
        id: 3,
        prompt: "Tim and Dan ... (play) basketball.",
        correctAnswers: [
          "Tim and Dan are playing basketball.",
          "Tim and dan are playing basketball.",
        ],
      },
      {
        id: 4,
        prompt: "Sue and her sister (drink) Coke.",
        correctAnswers: [
          "Sue and her sister are drinking Coke.",
          "Sue and her sister is drinking Coke.",
        ],
      },
      {
        id: 5,
        prompt: "Mike (sleep).",
        correctAnswers: ["Mike is sleeping."],
      },
    ],
  },

  // ── Exercise 4: Choose am / is / are ──────────────────────────────────────
  {
    kind: "chooseVerb",
    id: 4,
    ...
    questions: [
      {
        id: 1,
        sentence: "The baby (1) sleeping.",
        correctAnswers: ["is", "Is"],
      },
      {
        id: 2,
        sentence: "I (1) wearing a mac.",
        correctAnswers: ["am", "Am"],
      },
      {
        id: 3,
        sentence: "(1) you painting your face?",
        correctAnswers: ["are", "Are"],
      },
      {
        id: 4,
        sentence: "Mum and Dad (1) not eating hot dogs.",
        correctAnswers: ["are", "Are"],
      },
      {
        id: 5,
        sentence: "(1) your sister making a sandcastle?",
        correctAnswers: ["is", "Is"],
      },
    ],
  },
...
function inputBorderClass(state: AnswerState) {
...
function inputBgClass(state: AnswerState) {
  ...
  return (
    <div className="mt-3">
      ...
      <input
        ...
        className={`w-full rounded border-2 px-4 py-2.5 text-sm text-gray-800 outline-none transition focus:ring-2 ${inputBorderClass(state)} ${inputBgClass(state)}`}
      />
  ...
  return (
    <>
      ...
      <div className="bg-white border border-gray-200 shadow-sm">
        <div
          className="px-5 py-3 font-semibold text-sm text-gray-700"
          style={{ backgroundColor: "#fdf3e3", borderBottom: "1px solid #e5e7eb" }}
        >
          Picture
        </div>
        <div className="p-5 flex flex-col sm:flex-row gap-5 items-start">
          <img
            src={exercise.pictureSrc}
            alt="Exercise picture"
            className="w-60 flex-shrink-0 rounded object-cover border border-gray-200"
          />
          <p
            className="text-sm text-gray-700 leading-relaxed"
            dangerouslySetInnerHTML={{ __html: exercise.paragraphHtml }}
          />
        </div>
      </div>
      <div className="border-t border-gray-300" />
      {/* Questions */}
      <div className="bg-white border border-t-0 border-gray-200 shadow-sm px-5 pt-2 pb-6 space-y-0">
        {exercise.questions.map((q) => {
          ...
          return (
            <div key={q.id} className="border-b border-gray-200 pb-6 last:border-b-0 pt-4">
              <div
                className="mb-3 rounded-sm px-4 py-2"
...
function TranslateExercise({
  ...
  return (
    <div className="bg-white border border-gray-200 shadow-sm px-5 pt-2 pb-6 space-y-0">
      {exercise.questions.map((q) => {
        const val = answers[q.id] ?? "";
        const state: AnswerState = submitted ? checkAnswer(val, q.correctAnswers) : "idle";
        return (
          <div key={q.id} className="border-b border-gray-200 pb-6 last:border-b-0 pt-4">
            {/* Russian sentence as header */}
            <div
              className="mb-3 rounded-sm px-4 py-2.5"
              style={{ backgroundColor: "#fdf3e3", borderLeft: "4px solid #f59e0b" }}
            >
      ...
      {exercise.questions.map((q) => {
        ...
        return (
          <div key={q.id} className="border-b border-gray-200 pb-6 last:border-b-0 pt-4">
            ...
            <AnswerInput
              label="Впишите ответ для пропуска №1:"
              value={val}
              onChange={(v) => setAnswer(q.id, v)}
              state={state}
              correctAnswers={q.correctAnswers}
...
export default function App() {
  ...
  // answers[exerciseId][questionId] = string
  ...
  function calculateScore() {
    ...
    exercises.forEach((ex) => {
      ex.questions.forEach((q) => {
        total++;
        const val = allAnswers[ex.id]?.[q.id] ?? "";
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-lg p-10 max-w-md w-full text-center space-y-6">
          ...
          <button
            onClick={handleReset}
            className="mt-4 px-6 py-3 rounded-lg text-white font-semibold transition hover:opacity-90"
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-3xl mx-auto">
        {/* Header */}
        <div
          className="flex items-center justify-between px-5 py-4 rounded-t-md"
          style={{ backgroundColor: "#1e293b" }}
        >
          <span className="text-base font-medium text-white tracking-wide">
            `{exercise.title}`
          </span>
          <span className="text-sm font-semibold text-white whitespace-nowrap ml-4">
            {exerciseIndex + 1}/{totalExercises}
          </span>
        </div>

        {/* Exercise body — switch by kind */}
        ...
        <div className="flex items-center justify-between gap-3 pt-5">
          <button
            ...
            className="px-5 py-2.5 rounded-lg border border-gray-300 text-gray-600 text-sm font-medium hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed transition"
          >
            ← Назад
          </button>
